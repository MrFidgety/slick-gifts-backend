...
module Users
  RSpec.describe UserReceivedFriendRequestsQuery do
    describe '.all' do
      ...
      it 'returns all the user\'s received friend requests' do
        expect(subject).to match_array friend_requests
      end

      it 'does not return the user\'s sent friend requests' do
        requests = create_list(:friend_request, 2, user: user)
